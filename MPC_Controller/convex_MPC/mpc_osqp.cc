...
// X = A_qp * x_0 + B_qp * U
// costFunction = (X-Xd)^T * qp_weights * (X-Xd) + U^T * alpha * U
//              = [2 * Bqp^T * qp_weights * (Aqp * x0 - Xd)]^T * U + 
//                U^T * (Bqp^T * qp_weights * Bqp + alpha) * U
  ...
  if (qp_solver_name_ == OSQP) {
    ...
    if (workspace_ == 0) {
      ...
      int return_code = osqp_update_P_A(
          workspace_, objective_matrix_upper_triangle.valuePtr(), OSQP_NULL,
          nnzP, constraint_matrix.valuePtr(), OSQP_NULL, nnzA);
    ...
    qp_problem.init(red_hessian.data(), red_g_vec.data(), red_a_mat.data(),
                    ...
                    max_solver_iter);
    std::vector<qpOASES::real_t> qp_red_sol(new_vars, 0);
